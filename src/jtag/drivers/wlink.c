...
#define LIBUSB_RW_TIMEOUT	(3000)
	...
	#include <stdint.h>
	#include "hidapi.h"
	typedef int(__stdcall *pCH375OpenDevice)(uint32_t iIndex);
	typedef void(__stdcall *pCH375CloseDevice)(uint32_t iIndex);
	typedef uint32_t(__stdcall *pCH375SetTimeout)(uint32_t iIndex,
													uint32_t iWriteTimeout,
													uint32_t iReadTimeout);
	typedef uint32_t(__stdcall *pCH375ReadEndP)(uint32_t iIndex,
													uint32_t iPipeNum,
													void *iBuffer,
													uint32_t *ioLength);
	typedef uint32_t(__stdcall *pCH375WriteEndP)(uint32_t iIndex,
													uint32_t iPipeNum,
													void *iBuffer,
													uint32_t *ioLength);
	HMODULE hModule = 0;
	...
	int pWriteData(int dev, int endpoint, uint8_t *buf, uint32_t *length)
	{
		int ret;
		int pr=0;
		LOG_DEBUG(" ");
	...
	int pReadData(int dev, int endpoint, uint8_t *buf, uint32_t *length)
	{
		int ret;
		int pr=0;
		LOG_DEBUG(" ");
...
static void readmcause(void);
static void wlink_ramcodewrite(uint8_t *buffer, int size);
static void wlink_endprogram(void);
	...
	uint8_t *oOP)
{
	...
	Txbuf[2] = 0x06;
	Txbuf[3] = iAddr;
	Txbuf[4] = (uint8_t)(iData >> 24);
	Txbuf[5] = (uint8_t)(iData >> 16);
	Txbuf[6] = (uint8_t)(iData >> 8);
	Txbuf[7] = (uint8_t)(iData);
	Txbuf[8] = iOP;
	retrytime = 0;
// RETRY:
	len = 9;
...
uint8_t WriteNonFullPage(uint32_t iaddr,
							   uint8_t *ibuff,
							   uint32_t ilen)
{
	uint8_t Txbuf[pagesize];
	uint8_t Rxbuf[pagesize];
	uint32_t len;
	uint8_t i = 0;
	Txbuf[0] = 0x81;
	Txbuf[1] = 0x0A;
	Txbuf[2] = 0x05;
	iaddr += chipiaddr;
	Txbuf[3] = (uint8_t)(iaddr >> 24);
	Txbuf[4] = (uint8_t)(iaddr >> 16);
	Txbuf[5] = (uint8_t)(iaddr >> 8);
	Txbuf[6] = (uint8_t)(iaddr);
	Txbuf[7] = (uint8_t)ilen;
	len = 8;
...
int wlink_verify(uint32_t length, uint8_t *buffer)
{
	uint8_t txbuf[12];
	uint8_t rxbuf[12];
	uint32_t len = 4;
...
{	
	...
	if (pReadData(0, 1, rxbuf, &len))
	{
		...
		case 4:
			...
			break;
		default:
			wlink_name="unknown WCH-Link  mod:rv";
			break;
		}
		...
		switch (rxbuf[3])
		{
		case 0:  /** @FIXME Sharkey: With generic (unknown) WCH-Link I'm getting 0 for CH32V103C */
		{
			riscvchip = 0x01;
			...
			// if((rxbuf[2]==1)&&(rxbuf[3]==1)){
			// 	LOG_ERROR("Read-Protect Status Currently Enabled %02X %02X ", rxbuf[2], rxbuf[3]);
			// 	 return ERROR_FAIL;	
			// }
			...
			if(pReadData(0, 1, rxbuf, &len)){
					...
					uint8_t oOP;
					uint8_t iAddr=0;
					uint8_t Txbuf[52] = {0x13, 0x00, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00,
